...
public class DataProvideCreate {
	...
	public static String getLastCellData(String FileName, String SheetName, int rowNo, int CellNo) throws Exception {
		f = new File(FileName);
		fis = new FileInputStream(f);
		workbook = new XSSFWorkbook(fis);
		sheet = workbook.getSheet(SheetName);
	...
	public static Object[][] getData(String FileName, String SheetName) throws Exception // method creation
	{
		...
		int totalcols = getLastCellNum(FileName, SheetName, 1);
		Object[][] data1 = new Object[totalRows][totalcols]; // Object intialize arrays create
		for (int row = 1; row <= totalRows; row++) {
			for (int col = 0; col < totalcols; col++) {
				data1[row - 1][col] = getLastCellData(FileName, SheetName, row, col); // data insert Array
			}
		}
		return data1;
